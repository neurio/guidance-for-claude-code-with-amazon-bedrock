...
- **Silent Refresh for Azure AD**: Opt-in OIDC refresh token support eliminates browser re-authentication when AWS credentials expire
  - Add `"enable_silent_refresh": true` to your Azure AD profile in `config.json`
  - Requires `offline_access` permission on the Azure AD app registration
  - Refresh tokens stored securely via keyring or session files
  - Handles Azure AD refresh token rotation automatically
  - Non-Azure providers are unaffected
